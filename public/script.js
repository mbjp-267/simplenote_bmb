const noteForm = document.getElementById('noteForm');                        // Mengambil elemen form utama tempat menulis catatan (form tambah/edit catatan)
const titleInput = document.getElementById('title');                         // Mengambil inputan judul catatan
const contentInput = document.getElementById('content');                     // Mengambil inputan isi konten catatan
const notesContainer = document.getElementById('notesContainer');            // Tempat menampilkan semua catatan yang sudah dibuat
const searchInput = document.getElementById('searchInput');                  // Input pencarian untuk mencari catatan berdasarkan keyword
const filterDateInput = document.getElementById('dateFilter');               // Input filter berdasarkan tanggal catatan
const toggleDarkModeBtn = document.getElementById('toggleDarkMode');         // Tombol untuk mengganti mode terang ↔ gelap (dark mode)
const categoryInput = document.getElementById('category');                   // Input kategori saat membuat catatan baru
const filterCategoryInput = document.getElementById('categoryFilter');       // Input filter berdasarkan kategori catatan
const exportBtn = document.getElementById('exportBtn');                      // Tombol untuk mengekspor semua catatan (kemungkinan ke CSV atau JSON)
const importBtn = document.getElementById('importBtn');                      // Tombol untuk mengimpor catatan dari file
const importInput = document.getElementById('importInput');                  // Input file tersembunyi untuk memilih file saat impor
const PASSWORD_KEY = "notes_password";                                       // Kunci untuk menyimpan password di localStorage (sebagai proteksi catatan)
const defaultPassword = "1234";                                              // Password default jika belum pernah di-set (untuk akses awal)
const passwordOverlay = document.getElementById('passwordOverlay');          // Overlay yang muncul saat user harus memasukkan password
const passwordInput = document.getElementById('passwordInput');              // Inputan untuk mengetik password
const passwordSubmit = document.getElementById('passwordSubmit');            // Tombol untuk submit password
const passwordError = document.getElementById('passwordError');              // Elemen untuk menampilkan pesan error jika password salah
